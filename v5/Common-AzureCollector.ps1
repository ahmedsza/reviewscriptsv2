...
function Test-AzCliPrerequisites {
    ...
    # Some collectors call command groups that live in CLI extensions (for example
    # 'az monitor app-insights' needs 'application-insights'). With the default
    # 'yes_prompt' behaviour az asks for confirmation before installing, and because
    # the collector redirects the child process streams that prompt is invisible and
    # blocks the run forever. Force a non-interactive install decision instead.
    if (-not $env:AZURE_EXTENSION_USE_DYNAMIC_INSTALL) {
        $env:AZURE_EXTENSION_USE_DYNAMIC_INSTALL = 'yes_without_prompt'
    }
    if (-not $env:AZURE_CORE_DISABLE_CONFIRM_PROMPT) {
...
function Invoke-AzCommandJson {
    ...
    try {
        # Piping $null closes stdin for the child process so that any unexpected
        # interactive prompt fails fast instead of hanging the collection.
        $raw = $null | & az @fullArguments 2> $stderrFile
